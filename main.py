...
print('====================ENCAPSULATION: READING WITH PROPERTIES AND GETTERS====================')
...
print(f'Account 1 balance: {account.current_balance}')
...
account.account_type = 999
print(f'Account 1 type unchanged: {account.account_type}')

account.add_funds(-50)
account.add_funds('fifty')
print(f'Account 1 balance unchanged: {account.current_balance}')

account_2.withdraw(True)
account_2.withdraw(99999)
print(f'Account 2 balance unchanged: {account_2.current_balance}')
...
account_2.account_type = 'Everyday'
print(f'Account 2 new type: {account_2.account_type}')
...
print(f'Client 1 accounts: '
      f'{[acc.account_no for acc in client_1.get_accounts()]}')
print(f'Client 2 accounts: '
      f'{[acc.account_no for acc in client_2.get_accounts()]}')
...
print('====================COMPOSITION: ACCOUNT-CREATED TRANSACTIONS====================')

print(f'Account 1 has {len(account.transactions)} transaction record(s):')
for record in account.transactions:
    record.display_information()

print(f'Account 2 has {len(account_2.transactions)} transaction record(s):')
for record in account_2.transactions:
    print(record)

print(f'Account 3 has {len(account_3.transactions)} transaction record(s):')
for record in account_3.transactions:
    print(record)

records_before = len(account_3.transactions)
account_3.add_funds(-50)
account_3.withdraw(99999)
print(f'Account 3 still has {len(account_3.transactions)} transaction '
      f'record(s) after the rejected operations (was {records_before}).')

snapshot = account.transactions
snapshot.clear()
print(f'Account 1 still has {len(account.transactions)} transaction '
      f'record(s) after the copy was cleared.')
